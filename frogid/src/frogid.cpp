                                      ...
                                      Ptr<DescriptorMatcher>& descriptorMatcher )
{
	...
	descriptorMatcher = new cv::BFMatcher(NORM_L2, true);
                 ...
                 vector <Mat>& trainImages, vector<string>& trainImageNames )
{
    ...
    string trainDirName;
    readSetFilenames( trainFilename, trainDirName, trainImageNames );
...
int main(int argc, char** argv)
{
    ...
    string fileWithQueryImages, queryDirName;
    ...
    if( argc != 1 )
    {
        ...
        fileWithQueryImages = argv[4]; fileWithTrainImages = argv[5];
     ...
     if( !readImages( fileWithTrainImages, trainImages, trainImagesNames ) )
    {
	 ...
	 {
        string filename = queryDirName + queryImagesNames[i];
        Mat queryImage = imread( filename, CV_LOAD_IMAGE_GRAYSCALE );
		cout << "Query image: " << filename << endl;
